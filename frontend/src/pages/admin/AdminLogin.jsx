...
export default function AdminLogin() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      login({ token: res.data.token, user: res.data.user, role: 'admin' });
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 px-4 overflow-hidden">
      <motion.div
        className="absolute -top-24 -left-24 w-80 h-80 rounded-full bg-indigo-200/40 blur-3xl -z-10"
        animate={{ scale: [1, 1.2, 1] }}
        transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute -bottom-24 -right-24 w-80 h-80 rounded-full bg-purple-200/40 blur-3xl -z-10"
